...
module.exports = function(globalEventDispatch) {
    ...
    cliEventDispatch.on('cli.generateConfigured', function() {
    ...
    configHandler.on('configHandler.configCreated', function(command) {
        commands[command]();
    });
    configHandler.on('configHandler.readyToValidate', validators.validateConfig);
};
