...
const configDefault = path.normalize(`${process.cwd()}/test/acceptance/config.js`);
...
module.exports = generate = {
    configure(options) {
        ...
        if (components) {
            ...
            throw new MbttError.CLI.INVALID_COMPONENT_PATH(`Invalid component path: ${components}`);
        }
        ...
        if (technique.match(/^actionFocused$/)) {
            ...
            throw new MbttError.CLI.INVALID_GENERATION_TECHNIQUE(`Invalid test generation technique: ${technique}`);
        }
