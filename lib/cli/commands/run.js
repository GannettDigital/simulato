    ...
    configure(options) {
        ...
        if (saucelabs) {
            ...
            if (configFile.sauceCapabilities) {
                ...
                if (configFile.sauceCapabilities.hasOwnProperty('username')) {
                    process.env.SAUCE_USERNAME = configFile.sauceCapabilities.username;
                }
                if (configFile.sauceCapabilities.hasOwnProperty('accessKey')) {
                    process.env.SAUCE_ACCESS_KEY = configFile.sauceCapabilities.accessKey;
                }
            }
        }
        ...
        let rerun = options.rerunFailedTests || configFile.rerunFailedTests;
        if (rerun) {
            process.env.TEST_RERUN_COUNT = rerun;
        }
