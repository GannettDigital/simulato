...
module.exports = driverHandler = {
    ...
    _modifyObject(modifyingObject, modifiedObject) {
        if (typeof modifiedObject === 'undefined') {
            modifiedObject = {};
        }
        modifiedObject = modifyingObject;
    ...
    _navigateToAndModifyObject(modifyingObject, modifiedObject) {
        Object.keys(modifyingObject).forEach(function(k) {
            if (typeof modifyingObject[k] === 'object') {
                ...
                modifiedObject[k] = driverHandler._modifyObject(modifyingObject[k], modifiedObject[k]);
            }
        });
    },
    inSaucelabs() {
        ...
        if (process.env.SAUCE_CAPABILITIES) {
           capabilitiesList = JSON.parse(process.env.SAUCE_CAPABILITIES);
           driverHandler._navigateToAndModifyObject(capabilitiesList, capabilities);
        }
        const driver = new webdriver.Builder()
