...
module.exports = driverHandler = {
    ...
    inSaucelabs() {
        ...
        if (process.env.SAUCE_CONFIG) {
            let navigateObject = function(modifyingObject, modifiedObject) {
                Object.keys(modifyingObject).forEach(function(k) {
                    if (typeof modifyingObject[k] === 'object') {
                        navigateObject(modifyingObject[k], modifiedObject[k]);
                    } else {
           ...
           capabilitiesList = JSON.parse(process.env.SAUCE_CONFIG);
           navigateObject(capabilitiesList, capabilities);
        }
        const driver = new webdriver.Builder()
