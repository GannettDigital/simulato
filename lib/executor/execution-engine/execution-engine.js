...
module.exports = executionEngine = {
    ...
    configure(testCaseActions) {
        ...
        executionEngine.emit('executionEngine.createExpectedState', function(expectedState) {
    ...
    _executeStep() {
        ...
        switch (executionEngine._step) {
            case 'preconditions':
                let preconditions;
                if (actionParameters) {
                    preconditions = executionEngine._action.preconditions(...actionParameters);
                } else {
            ...
            case 'perform':
                if (actionParameters) {
                    try {
                        executionEngine._action.perform(...actionParameters, callback);
                    } catch (error) {
                    ...
                    try {
                        executionEngine._action.perform(callback);
                    } catch (error) {
                        error.message = `The error '${error.message}' was thrown while ` +
                        `executing the perform function for ` +
                        `'${executionEngine._actionConfig.instanceName}' ` +
            ...
            case 'effects':
                if (actionParameters) {
                    try {
                        executionEngine._action.effects(...actionParameters, executionEngine._expectedState);
                    } catch (error) {
                        error.message = `The error '${error.message}' was thrown while ` +
                        `executing the perform function for ` +
                        `'${executionEngine._actionConfig.instanceName}' ` +
                    ...
                    try {
                        executionEngine._action.effects(executionEngine._expectedState);
                    } catch (error) {
                        error.message = `The error '${error.message}' was thrown while ` +
                        `executing the perform function for ` +
                        `'${executionEngine._actionConfig.instanceName}' ` +
