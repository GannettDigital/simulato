...
module.exports = eeReportHandler = {
  ...
  endAction() {
    const action = eeReportHandler._report.actions[eeReportHandler._currentActionIndex];
  ...
  endStep(error, stepName, expectedState) {
    const step = eeReportHandler._report.actions[eeReportHandler._currentActionIndex].steps[stepName];
    step.time = process.hrtime(step.time);
    if (stepName === 'effects' && (configHandler.get('reportStates') || error)) {
      step.expectedState = JSON.parse(JSON.stringify(expectedState._state));
    }
    if (error) {
      if (stepName === 'effects') {
        step.pageState = JSON.parse(JSON.stringify(expectedState._pageState));
