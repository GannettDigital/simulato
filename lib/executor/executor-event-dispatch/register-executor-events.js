...
module.exports = function(executorEventDispatch) {
  ...
  executionEngine.on('executionEngine.preconditionsReady', assertionHandler.assertPageState);
  executionEngine.on('executionEngine.effectsApplied', assertionHandler.assertExpectedPageState);
  executionEngine.on('executionEngine.createExpectedState', function(callback) {
    executorEventDispatch.emit('executor.createExpectedState', callback);
  });
