...
module.exports = assertionHandler = {
  ...
  * _getAndCheckPageState() {
    ...
    const preconditions = yield assertionHandler.emitAsync('assertionHandler.cloneAndGetPreconditions', next);
  ...
  * _getAndCheckExpectedPageState() {
  ...
  _cloneAndApplyEffects() {
    ...
    const component = assertionHandler._clonedExpectedState.getComponent(assertionHandler._actionConfig.name);
    try {
  ...
  _cloneAndGetPreconditions(callback) {
    ...
    assertionHandler.emit('assertionHandler.preconditionsCalculated', preconditions);
    return callback(null, preconditions);
  },
  _clone() {
    const clonedDataStore = assertionHandler._dataStore.clone();
