...
module.exports = function(elements, instanceName, componentName) {
  ...
  for (let [index, value] of elements.entries()) {
    ...
    if (typeof value.selector !== 'object' || Array.isArray(value.selector)) {
      throw new MbttError.ELEMENT.ELEMENT_OBJECT_PROPERTY_TYPE(
        `The selector field for '${value.name}' must be an object inside component '${componentName}'`
      );
    }
    if (typeof value.selector.type !== 'string') {
      throw new MbttError.ELEMENT.ELEMENT_OBJECT_PROPERTY_TYPE(
        `The selector 'type' field for '${value.name}' must be a string inside component '${componentName}'`
      );
    }
    if (!(value.selector.type === 'attribute' || value.selector.type === 'querySelector')) {
      throw new MbttError.ELEMENT.ELEMENT_OBJECT_PROPERTY_TYPE(
        `The selector 'type' field for '${value.name}' must be either 'attribute' or 'querySelector inside component '${componentName}'`
      );
    }
    if (value.selector.type === 'attribute' && typeof value.selector.key !== 'string') {
      throw new MbttError.ELEMENT.ELEMENT_OBJECT_PROPERTY_TYPE(
        `The selector 'key' field for '${value.name}' must be a string when using selector type attribute inside component '${componentName}'`
      );
    }
    if (typeof value.selector.value !== 'string') {
      throw new MbttError.ELEMENT.ELEMENT_OBJECT_PROPERTY_TYPE(
        `The selector 'value' field for '${value.name}' must be a string inside component '${componentName}'`
      );
    }
  }
};
