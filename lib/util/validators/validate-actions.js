...
module.exports = validateActions = {
  validate(actions, instanceName, componentName) {
    if (!(typeof actions === 'object') || Array.isArray(actions)) {
      throw new MbttError.ACTION.ACTIONS_NOT_OBJECT(
    ...
    for (let actionName in actions) {
      if (actions.hasOwnProperty(actionName)) {
        ...
        if (!(typeof action === 'object') || Array.isArray(action)) {
          throw new MbttError.ACTION.ACTION_TYPE_ERROR(
            `Action '${actionName}' for '${instanceName}' is not an object`
          );
        }
