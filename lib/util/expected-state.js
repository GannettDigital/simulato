...
module.exports = ExpectedState = {
  ...
  clone(dataStore, callback) {
    ...
    this.create(dataStore, (expectedState) => {
      ...
      if (process.env.PLANNING_MODE) {
        clonedState._planningStashedStates = _.cloneDeep(this._planningStashedStates);
      }
  ...
  _cloneComponent(component) {
    const componentOptions = _.cloneDeep(component.options);
  ...
  _cloneAndAddComponents(components, state) {
    for (const [type, component] of components.entries()) {
      const componentState = _.cloneDeep(state[type]);
      const componentOptions = _.cloneDeep(component.options);
  ...
  _cloneStashedComponents(stashedComponents) {
    for (const stashedComponentMap of stashedComponents) {
      const components = new Map();
      for (const component of stashedComponentMap.values()) {
        const myComponent = this._cloneComponent(component);
  ...
  _cloneStashedDynamicAreas(stashedDynamicAreas) {
    for (const dynamicArea of stashedDynamicAreas) {
      const clonedDynamicArea = new Map();
      for (const [dynamicAreaName, components] of dynamicArea.entries()) {
  ...
  _cloneStashedDynamicAreasComponentsAndStates(stashedDynamicAreasComponentsAndStates) {
    for (const [dynamicAreaName, stashedData] of stashedDynamicAreasComponentsAndStates.entries()) {
      const components = new Map();
      for (const component of stashedData.components.values()) {
        const clonedComponent = this._cloneComponent(component);
        components.set(clonedComponent.name, clonedComponent);
      }

      const states = new Map();
      for (const [componentName, state] of stashedData.states) {
  ...
  createComponent(componentConfig) {
    const {type, name, options, dynamicArea} = componentConfig;
    let newComponent;
  ...
  addComponent(component, state, cloning) {
    ...
    if (process.env.PLANNING_MODE) {
      this._newComponents.push(component.name);
    }
  ...
  _addChildren(component) {
    for (const child of component.children) {
      ...
      const childComponent = this.createComponent(child);
    ...
    if (component.events) {
      for (const event of component.events) {
        if (Array.isArray(event.name)) {
          for (const eventName of event.name) {
  ...
  createAndAddComponent(componentConfig) {
    const newComponent = this.createComponent(componentConfig);
  ...
  delete(name) {
    ...
    if (component) {
      for (const dynamicArea of this._dynamicAreas.values()) {
  ...
  clearDynamicArea(dynamicArea) {
    if (this._dynamicAreas.get(dynamicArea)) {
      const components = Array.from(this._dynamicAreas.get(dynamicArea));
  ...
  stash() {
    ...
    if (process.env.PLANNING_MODE) {
      this._popCount = 0;
      this._planningStashedStates.push(_.cloneDeep(state));
    }
  ...
  stashDynamicArea(dynamicArea) {
    const stashedDynamicAreaComponents = new Map();
    const stashedDynamicAreaStates = new Map();
    let dynamicAreaComponentNames;
    if (this._dynamicAreas.get(dynamicArea)) {
      dynamicAreaComponentNames = this._dynamicAreas.get(dynamicArea);
      for (const dynamicAreaComponentName of dynamicAreaComponentNames) {
        const component = this.getComponent(dynamicAreaComponentName);
