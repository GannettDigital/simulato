...
module.exports = ExpectedState = {
    ...
    clone(dataStore, callback) {
        ...
        this.create(dataStore, (expectedState) => {
            ...
            this._stashedDynamicAreasComponentsAndStates.forEach(function(value,key,map){
                let clonedDynamicAreaComponentAndState = new Map();
                for (let [dynamicAreaName, components] of value.entries()) {
                    clonedDynamicAreaComponentAndState.set(
                        dynamicAreaName, 
                        {
                            components: this._stashedDynamicAreasComponentsAndStates.get(dynamicAreaName).components,
                            states: this._stashedDynamicAreasComponentsAndStates.get(dynamicAreaName).states,
                        });
                }
    ...
    stashDynamicArea(dynamicArea){
        ...
        let stashedDynamicAreaStates = new Map();
            this._dynamicAreas.get(dynamicArea).forEach(
                function(componentName) {
                    let component = this.getComponent(componentName);
                    stashedDynamicAreaComponents.set(componentName, component);
                    stashedDynamicAreaStates.set(componentName, this._state[component.type]);
                }
            );
