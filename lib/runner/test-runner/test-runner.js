...
module.exports = testRunner = {
    ...
    _startTest(spawnArgs, testPath) {
        ...
        test.on('close', function(exitCode) {
            ...
            testRunner.emit('testRunner.testFinished', test.number);
            if (testRunner._testsRemaining === 0) {
                if (testRunner._testsToRerun.length === 0 || testRunner._rerunCount <= 0) {
                    ...
                    testRunner.__testFiles = testRunner._testsToRerun;
                    testRunner._testsToRerun = [];
                    testRunner._testsRemaining = testRunner.__testFiles.length;
                    testRunner._rerunCount--;
                    testRunner.emit('testRunner.testsReadyToRerun');
                }
            }
        });
    ...
    _configureRerun() {
        //do all the stuff to handle the tool knowing tests need to rerun
...
testRunner.on('testRunner.testsReadyToRerun', testRunner._configureRerun);
