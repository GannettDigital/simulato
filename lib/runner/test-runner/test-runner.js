...
module.exports = testRunner = {
  ...
  _startTest(spawnArgs, testPath) {
    ...
    const test = childProcess.spawn('node', spawnArgs, {
      env,
  ...
  _scheduleTests() {
    const numberOfTestsToStart = testRunner._parallelism - testRunner._testsRunning;
    for (let i = 0; i < numberOfTestsToStart; i++) {
      const testFile = testRunner._testFiles.pop();
      if (testFile) {
        const now = Date.now();
