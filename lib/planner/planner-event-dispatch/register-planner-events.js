...
const actionTree = require('../search-algorithms/action-tree.js');
const actionTree2 = require('../search-algorithms/action-tree-v2-electric-boogaloo.js');
const entryComponents = require('../entry-components.js');
const stateDifference = require('../state-difference.js');

module.exports = function(plannerEventDispatch) {
  plannerEventDispatch.on('planner.generateConfigured', testPlanner.generateTests);

  plannerEventDispatch.runOn('testPlanner.createPlans', actionTree.createPlans);
  // plannerEventDispatch.runOn('testPlanner.createPlans', actionTree2.createPlans);
  ...
  plannerEventDispatch.runOn('possibleActions.checkIfPossible', possibleActions.checkIfPossible);

  plannerEventDispatch.runOn('entryComponents.get', entryComponents.get);

  plannerEventDispatch.on('stateDifference.createDiff', stateDifference.createDiff);
};
