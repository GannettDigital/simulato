...
module.exports = forwardStateSpaceSearch = {
    * _createPlans(entryComponents, predeterminedGoalAction, callback) {
        ...
        for (let type of entryComponents) {
            ...
            startNode.actions = yield forwardStateSpaceSearch
                .emit('forwardStateSpaceSearch.getPossibleActions', startNode, next);
            yield forwardStateSpaceSearch
                .emit('forwardStateSpaceSearch.addActions', planningProblem, startNode.actions, next);
    ...
    _addActions(planningProblem, actions, callback) {
        let {goalActions, discoveredActions} = planningProblem;
    ...
    * _addApplicableActions(planningProblem, node, callback) {
        ...
        for (let component of components) {
            for (let actionName of Object.getOwnPropertyNames(component.actions)) {
                ...
                if (applicable === true && node.path.indexOf(actionIdentifier) === -1) {
    ...
    * _findGoalActions(planningProblem) {
        ...
        for (let [key, node] of fringe.entries()) {
            ...
            for (let action of node.actions.values()) {
                ...
                clonedNode.actions = yield forwardStateSpaceSearch.
                    emit('forwardStateSpaceSearch.getPossibleActions', clonedNode, next);
                yield forwardStateSpaceSearch.
                    emit('forwardStateSpaceSearch.addActions', planningProblem, clonedNode.actions, next);
    ...
    _applyActionToNode(node, callback) {
        ...
        let action = node.state.getComponentsAsMap().get(name).actions[actionName];
...
forwardStateSpaceSearch.on('forwardStateSpaceSearch.addActions',
    function(planningProblem, actions, callback) {
        process.nextTick(function() {
            forwardStateSpaceSearch._addActions(planningProblem, actions, callback);
        });
    });
