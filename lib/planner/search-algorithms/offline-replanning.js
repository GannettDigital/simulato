  ...
  * _checkForLoopAndBackTrack(plan, callback) {
    ...
    if (loopStartIndex !== -1) {
      ...
      if (backtrackPlan !== null) {
        ...
        return callback(new Error('Unable to backtrack!'));
  ...
  * _updateExistingPlansAndActionOccurrences(callback) {
  ...
  _savePlan(plan) {
    ...
    if (configHandler.get('debug')) {
      ...
      console.log(offlineReplanning._satisfiedActions.size);
    }
  },
  _getAction(plan) {
  ...
  _filterZeroActionCountActions(unusedActions) {
    ...
    for (let action of unusedActions) {
      let actionCount = offlineReplanning._actionOccurrences.get(action);
      if (actionCount > 0 ) {
  ...
  _getMostOccurringAction(possibleActions) {
    ...
    for (let action of possibleActions) {
      ...
      let occurrences = offlineReplanning._actionOccurrences.get(action);
