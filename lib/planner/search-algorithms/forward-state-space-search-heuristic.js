...
module.exports = forwardStateSpaceSearch = {
  ...
  * createPlans(callback) {
    ...
    const startNodes = yield forwardStateSpaceSearch.emitAsync('startNodes.get', next);

    for (const startNode of startNodes) {
      forwardStateSpaceSearch._addActions(startNode.allActions);
      const unfoundGoalActionCount = forwardStateSpaceSearch._findUnfoundGoalActionCount(startNode);
  ...
  _addActions(actions) {
    const {_goalActions, _discoveredActions} = forwardStateSpaceSearch;
  ...
  * _findGoalActions() {
    ...
    const {_goalActions, _discoveredActions, _foundGoalActions, _predeterminedGoalAction, _callback} =
      forwardStateSpaceSearch;

    const node = forwardStateSpaceSearch._getNodeFromMap();
    ...
    for (const action of node.actions.values()) {
      const clonedNode = yield forwardStateSpaceSearch
      ...
      const {applicableActions, allActions} = yield forwardStateSpaceSearch
      ...
      const unfoundGoalActionCount = forwardStateSpaceSearch._findUnfoundGoalActionCount(clonedNode);
  ...
  * _testForGoal(node, callback) {
    ...
    const {_goalActions, _foundGoalActions, _predeterminedGoalAction} = forwardStateSpaceSearch;
  ...
  _setNodeInMap(node, unfoundGoalActionCount) {
    const {_pathArray} = forwardStateSpaceSearch;
  ...
  _findNextNode(nodesArray, currentActionsCount) {
    ...
    while (nodesArray.length) {
      ...
      const unfoundGoalActionCount = forwardStateSpaceSearch._findUnfoundGoalActionCount(node);
    ...
    for (const reorderNode of nodesToReorder) {
