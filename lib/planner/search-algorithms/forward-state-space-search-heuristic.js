...
module.exports = forwardStateSpaceSearch = {
    ...
    * _createPlans(entryComponents, predeterminedGoalAction, callback) {
        ...
        for (let componentName of entryComponents) {
            ...
            startNode.state.createAndAddComponent(componentName, component.entryComponent.name,
                component.entryComponent.state, component.entryComponent.options);
    ...
    * _findGoalActions() {
    ...
    _applyActionToNode(node, callback) {
        ...
        if (Array.isArray(action.parameters)) {
            ...
            action.effects.call(node.state.getComponent(instanceName), ...testCaseAction.options.parameters, node.state);
        } else {
            action.effects.call(node.state.getComponent(instanceName), node.state);
        }
    ...
    * _checkPreconditions(node, action, actionIdentifier, callback) {
        ...
        let preconditions;
        try {
            preconditions = action.preconditions.call(node.state.getComponent(actionIdentifier.split('.')[0])); // Bug that it is not called with parameters?
        } catch (error) {
            error.message =
