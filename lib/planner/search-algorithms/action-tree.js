...
module.exports = actionTree = {
  _id: 1,
  _visitedIds: new Set(),
  _foundPredeterminedGoalActionPath: false,
  ...
  * createPlans(callback) {
    ...
    for (const rootActionId of actionTree._actionStack) {
      const rootAction = actionTree._allNodes.get(rootActionId);
      const rootNode = actionTree._rootNodes.get(rootAction.name);
    ...
    if (actionTree._predeterminedGoalAction && !actionTree._foundPredeterminedGoalActionPath) {
      throw new Error(`Could not find predetermined action ${actionTree._predeterminedGoalAction}`);
    }
  ...
  * _performActions(callback) {
    ...
    while (actionTree._actionStack.length) {
      ...
      const nextAction = actionTree._actionStack.pop();
  ...
  * _performAction(actionNode, callback) {
    ...
    const path = actionTree._getActionPath(actionTree._allNodes.get(actionNode));

    // Init the expected state and datastore
    yield actionTree._initExpectedState(next);

    const rootComponent = yield actionTree.emitAsync('componentHandler.getComponent', path[path.length - 1].type, next);
    ...
    if (actionTree._predeterminedGoalAction == `${path[0].name}.${path[0].action}`) {
      actionTree._actionStack = [];
      actionTree._foundPredeterminedGoalActionPath = true;
    ...
    const oldDatastore = JSON.parse(JSON.stringify(actionTree._dataStore.retrieveAll()));
    ...
    const touchedNodesInPreconMap = new Set();
    // Get difference in states
    const stateDiff = yield actionTree.emitAsync('stateDifference.createDiff',
        oldState, actionTree._expectedState.getState(), next);
    ...
    const dataDiff = yield actionTree.emitAsync('stateDifference.createDiff',
        oldDatastore, actionTree._dataStore.retrieveAll(), next);
    ...
    for (const componentName of actionTree._expectedState.getNewComponents()) {
  ...
  * _processEntryComponents(callback) {
    const next = yield;
    const entryComponents = yield actionTree.emitAsync('entryComponents.get', next);
  ...
  * _processNewComponent(componentName, parentNode, callback) {
    const next = yield;
    const component = actionTree._expectedState.getComponent(componentName);
    if (component) {
      for (const actionKey in component.actions) {
  ...
  * _processAction(component, actionName, parentNode, callback) {
    ...
    if (!actionTree._visitedActions.has(actionIdentifier)) {
      ...
      const possibilityData = yield actionTree.emitAsync(
      ...
      const actionNodePreconditions = [];
      if (possibilityData.preconditions) {
        for (const precondition of possibilityData.preconditions) {
      ...
      const actionNode = actionTree._createNode(component, actionName, parentNode, possibilityData.applicable);

      if (possibilityData.applicable) {
        // add to actionStack to get performed
        actionTree._actionStack.push(actionNode.id);
      } else {
        // Add each prcondition to the unmetPrecons
  ...
  _addToPreconMap(actionNode, precondition) {
    ...
    const nodesToMeetPrecon = actionTree._getNodesToMeetPrecondition(precondition.key, precondition.value);
    ...
    const unmetPrecon = actionTree._unmetPreconditions[precondition.key];
    ...
    for (const setter of nodesToMeetPrecon) {
  ...
  _applyActionEffects(actionNode, callback) {
    const component = actionTree._expectedState.getComponent(actionNode.name);
    const action = component.actions[actionNode.action];
  ...
  _insertIntoDataSet(entry, actionNode, touchedNodesInPreconMap) {
    ...
    if (actionTree._unmetPreconditions.hasOwnProperty(entry.key)) {
      ...
      const unmetPrecon = actionTree._unmetPreconditions[entry.key];
      ...
      if (preconValueIndex > -1 || specialPropertyCase) {
        ...
        for (const touchedNode of actionTree._unmetPreconditions[entry.key].actionNodes[preconValueIndex]) {
  ...
  * _processTouchedNodes(touchedNodes, callback) {
    ...
    for (const node of touchedNodes.values()) {
      // First check if all precons are met for this node
      let possible = true;
      const nodesToHit = [];
      const valueIndexs = [];
      const allPrecons = actionTree._discoveredActions.get(node).preconditions;
      for (let i = 0; i < allPrecons.length; i++) {
        const precon = allPrecons[i];
        ...
        if (actionTree._unmetPreconditions.hasOwnProperty(precon.key)) {
          const unmetPrecon = actionTree._unmetPreconditions[precon.key];
      ...
      if (possible) {
        const nodePath = yield actionTree.emitAsync('actionTree.constructPath', node, nodesToHit, next);
  ...
  _processNewPath(nodePath) {
    ...
    for (i; i < nodePath.length; i++) {
      if (nodePath[i].parent) {
        let flag = true;
        for (const child of nodePath[i - 1].children) {
    ...
    for (i; i < nodePath.length - 1; i++) {
      const node = actionTree._createNode(nodePath[i], nodePath[i].action, nodePath[i - 1], true);
      nodePath[i] = node;
    }
  ...
  _removeFromPreconMap(node, preconditions, valueIndexs) {
    ...
    for (let i = 0; i < preconditions.length; i++) {
      const unmetPrecondition = actionTree._unmetPreconditions[preconditions[i].key];
      // remove from actionNodes
      const actionNodes = unmetPrecondition.actionNodes[valueIndexs[i]];
  ...
  * _constructPath(actionNode, nodesToHit, callback) {
    ...
    // Plug all notes to hit into a set for use with short circuit logic
    const uniqueNodesToHit = new Set();

    // Construct a path to the actionNode, making sure to hit all nodes needed to meet preconditions
    const actions = new Map();
    for (const nodeIdentifiers of nodesToHit) {
      for (const nodeIdentifier of nodeIdentifiers) {
        const nodes = actionTree._discoveredActions.get(nodeIdentifier).nodes;
        for (const node of nodes) {
          uniqueNodesToHit.add(node);
          actionTree._getActionsFromPath(actionTree._allNodes.get(node), actions);
        }
      }
    }
    let rootNode;
    const nodes = actionTree._discoveredActions.get(actionNode).nodes;
    for (const node of nodes) {
      rootNode = actionTree._getActionsFromPath(actionTree._allNodes.get(node), actions);
    }

    // Check the short circuit logic to see if we can
    // easily add actionNode to the end to actions that meet our preconditions
    for (const uniqueNode of uniqueNodesToHit) {
      // Init state
      yield actionTree._initExpectedState(next);

      const rootComponent = yield actionTree.emitAsync('componentHandler.getComponent', rootNode.type, next);
      actionTree._processEntryComponent(rootComponent);

      // recreate state up to uniqueNode
      const uniqueNodePath = actionTree._getActionPath(actionTree._allNodes.get(uniqueNode));

      // Apply Effects on all all nodes down the path
      for (let i = uniqueNodePath.length - 1; i >= 0; i--) {
        yield actionTree.emitAsync('actionTree.applyActionEffects', uniqueNodePath[i], next);
      }

      // see if our actionNode is possible
      const {applicableActions} = yield actionTree.emitAsync('possibleActions.get', actionTree._expectedState, next);

      if (applicableActions.has(actionNode)) {
        // We found an easy path to our node!
        // Get the actionNode from actions and push it to the path we are done!
        for (const action of actions.values()) {
          if (action.actionIdentifier === actionNode) {
            // Set this path into the path to be returned by this function, need to reverse the current path
            path = uniqueNodePath.reverse();
            path.push(action);
            break;
          }
        }
        if (configHandler.get('debug') === true) {
          console.log(`Found short circuit!`);
    ...
    // Try and string all unique nodes' actions together to reach action
    // Basically a simplified, one-shot of offline replanning algorithm
    if (path.length === 0) {
      const trees = new Map();
      const uniqueActions = new Map();
      for (const uniqueNode of uniqueNodesToHit) {
        const actionPath = actionTree._getActionPath(actionTree._allNodes.get(uniqueNode));
        for (const action of actionPath) {
          uniqueActions.set(action.actionIdentifier, action.id);
        }
        // const uniqueActions = uniqueActions.add(actionTree._allNodes.get(uniqueNode).action);
      }
      yield actionTree._initExpectedState(next);
      const rootComponent = yield actionTree.emitAsync('componentHandler.getComponent', rootNode.type, next);
      actionTree._processEntryComponent(rootComponent);
      ...
      // Create root nodes
      for (const applicableAction of applicableActions) {
        // Only add root node if it is in the list of unique actions
        if (uniqueActions.has(applicableAction)) {
          const actionsLeft = new Map(uniqueActions);
          actionsLeft.delete(applicableAction);
          trees.set(applicableAction, [{
            parent: null,
            action: actionTree._allNodes.get(uniqueActions.get(applicableAction)),
            depth: 0,
            actionsLeft,
          }]);
        }
      ...
      let discoveredPathIndex = -1;
      let treeIndex = 0;
      for (; treeIndex < trees.size && discoveredPathIndex === -1; treeIndex++) {
        const tree = trees.get([...trees.keys()][treeIndex]);
        // Tack if actions was executed

        // Initialize state to start the search
        yield actionTree._initExpectedState(next);
        const rootComponent = yield actionTree.emitAsync('componentHandler.getComponent', tree[0].action.type, next);
        actionTree._processEntryComponent(rootComponent);
        yield actionTree.emitAsync('actionTree.applyActionEffects', tree[0].action, next);

        let index = 0;
        let thereIsProgressToBeMade = true;
        while (thereIsProgressToBeMade) {
          const {applicableActions} =
            yield actionTree.emitAsync('possibleActions.get', actionTree._expectedState, next);

          // Check if desired action can be executed
          if (applicableActions.has(actionNode)) {
            discoveredPathIndex = index + 1;
            tree.push({
              parent: tree[index],
              action: actionTree._allNodes.get(actionTree._discoveredActions.get(actionNode).nodes[0]),
              actionsLeft: new Set(),
              depth: tree[index].depth + 1,
            });
            break;
          }
          ...
          for (const applicableAction of applicableActions) {
            if (tree[index].actionsLeft.has(applicableAction)) {
              const action = actionTree._allNodes.get(uniqueActions.get(applicableAction));
              yield actionTree.emitAsync('actionTree.applyActionEffects', action, next);
              const actionsLeft = new Map(tree[index].actionsLeft);
              actionsLeft.delete(applicableAction);
              tree.push({
                parent: tree[index],
                action,
          ...
          if (!foundAction) {
            thereIsProgressToBeMade = false;
          }
        }
      }

      if (discoveredPathIndex !== -1) {
        const tree = trees.get([...trees.keys()][treeIndex - 1]);
        path = [];
        let node = tree[discoveredPathIndex];
        for (let i = tree[discoveredPathIndex].depth; i >= 0; i--) {
    ...
    if (path.length === 0) {
      ...
      const pathsActions = new Map(actions);
      for (const possibleAction of applicableActions.values()) {
        for (const action of actions.values()) {
          if (possibleAction === action.actionIdentifier) {
            const actionsLeft = new Map(pathsActions).delete(action.id);
            tree.push({
              parent: null,
              action,
              depth: 0,
      ...
      while (actionNotFound && pathValid) {
        ...
        for (let i = treeStartIndex; i < target; i++) {
          ...
          const path = actionTree._getActionPath(node);
          ...
          for (const possibleAction of applicableActions.values()) {
            for (const action of pathsActions.values()) {
              if (possibleAction === action.actionIdentifier) {
                const actionsLeft = new Map(pathsActions);
        ...
        if (countOfNodesChecked == 0) {
          // Could not find path with the nodes to hit provided.
          // Still might be possible with other nodes to meet (mutiple actions can affect the same precondition)
          pathValid = false;
        }
  ...
  _createTestPaths() {
    ...
    for (const rootNode of actionTree._rootNodes.values()) {
      for (const rootActionId of rootNode.actionNodes) {
        const rootAction = actionTree._allNodes.get(rootActionId);
  ...
  _createFinalPath(node, depth) {
    ...
    const path = [];
    const actionId = node.actionIdentifier;
    ...
    for (let i = depth; i >= 0; i--) {
      ...
      // Leave a space at the beginning for the entry stuff, so depth + 1
      path[i + 1] = action;
      if (i === 0) {
        // get entry component to get the state
        const rootComponent = actionTree._rootNodes.get(node.name).component;
        path[0] = {
    ...
    if (actionId == actionTree._predeterminedGoalAction) {
      actionTree._testPaths.push(path);
    } else if (!actionTree._predeterminedGoalAction) {
      actionTree._testPaths.push(path);
    }
  },
  * _checkForLeftoverPossibleActions(callback) {
    ...
    for (const key in actionTree._unmetPreconditions) {
      if (actionTree._unmetPreconditions.hasOwnProperty(key)) {
        const precondition = actionTree._unmetPreconditions[key];
        ...
        for (let i = 0; i < precondition.values.length; i++) {
          ...
          if (precondition.setters[i].length > 0) {
            for (const node of precondition.actionNodes[i]) {
  ...
  _checkForMatchingConditions(precondition, preconValueIndex, preconKey, touchedNodes, fullUmetPrecondition) {
    ...
    if (precondition[0] === 'property') {
      if (actionTree._metaState.hasOwnProperty(preconKey)) {
        for (const nodeArr of actionTree._metaState[preconKey].actionNodes) {
      ...
      if (actionTree._metaState.hasOwnProperty(preconKey)) {
        ...
        if (valueIndex > -1) {
          ...
          if (actionTree._metaState[preconKey].actionNodes[valueIndex].length > 0) {
            ...
            for (const actionNode of actionTree._metaState[preconKey].actionNodes[valueIndex]) {
              // Add each actionNode to the corresponding actionTree._unmetPrecondition
              actionTree._unmetPreconditions[preconKey].setters[preconValueIndex].push(actionNode.actionIdentifier);
            }
            for (const touchedNode of precondition.actionNodes[preconValueIndex]) {
