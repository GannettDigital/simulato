...
module.exports = actionTree = {
  ...
  * createPlans(callback) {
    ...
    // console.log(c.size); //a\b => {1}
    // for (let action of c.values()) {
    //   console.log(action);
    // }
  ...
  * _constructPath(actionNode, nodesToHit, callback) {
    ...
    while (actionNotFound && pathValid) {
      //  TODO: Need to break loop when not solved else infinite
      ...
      if (countOfNodesChecked == 0) {
        // DID NOT FIND PATH!!!!!!! Try again later could be still solved
        // throw new Error(`Cannot find a path for ${actionNode}`);
        pathValid = false;
      }
