...
module.exports = actionTree = {
  ...
  * _processTouchedNodes(touchedNodes, callback) {
  ...
  _processNewPath(nodePath) {
    ...
    /*
      TODO: Once nodes start getting cloned below,
      they are making unique actions in a unique state that they were not previously doing
      Do we want to take account for this,? check if it effects the state in a new way?
      Adding to metaES and unmetPrecons
    ...
    */
    ...
    // // Travel down the path, until we find a node out of place
    // //skip root, and dont do the final action (target action)
  ...
  * _constructPath(actionNode, nodesToHit, callback) {
    ...
    // Check the sort circuit logic to see if we can
    ...
    if (path.length === 0) {
      ...
      while (actionNotFound && pathValid) {
        // TODO: IF time goes over a limit, such as like 5 seconds
        // fallback to a DFS less efficient algo or maybe based on action count?
        ...
        if (countOfNodesChecked == 0) {
          ...
          pathValid = false;
          // TODO: Figure out what precondition was not met and the action we thought were meet it.
          // This action does not need to be a setter for our particular actionNode
          // however unmetPrecons is not set up to handle that additional information,
          // but will make future path contrusctions faster if it did know to not even try
        }
