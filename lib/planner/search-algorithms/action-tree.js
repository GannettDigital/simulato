...
module.exports = actionTree = {
  ...
  * _performAction(actionNode, callback) {
    ...
    let stateDiff = yield actionTree.emitAsync('stateDifference.createDiff',
    ...
    let dataDiff = yield actionTree.emitAsync('stateDifference.createDiff',
  ...
  * _constructPath(actionNode, nodesToHit, callback) {
    ...
    for (let uniqueNode of uniqueNodesToHit) {
      // Init state
      yield actionTree._initExpectedState(next);
      ...
      let uniqueNodePath = actionTree._getActionPath(actionTree._allNodes.get(uniqueNode));
      ...
      let {applicableActions} = yield actionTree.emitAsync('possibleActions.get', actionTree._expectedState, next);
    ...
    if (path.length === 0) {
      ...
      for (; treeIndex < trees.size && discoveredPathIndex === -1; treeIndex++) {
        ...
        yield actionTree._initExpectedState(next);
        let rootComponent = yield actionTree.emitAsync('componentHandler.getComponent', tree[0].action.type, next);
        actionTree._processEntryComponent(rootComponent);
  ...
  _createFinalPath(node, depth) {
    ...
    let path = [];
    let actionId = node.actionIdentifier;
