...
module.exports = actionTree = {
  ...
  * _performActions(callback) {
    ...
    while(actionTree._actionStack.length) {
      ...
      if (!actionTree._visitedIds.has(nextAction)) {
        ...
        console.log('already did this id');
      }
    }
    // Stack is now empty
  ...
  * _processTouchedNodes(touchedNodes, callback) {
  ...
  _processNewPath(nodePath) {
    ...
    // for (let j = 0; j < nodePath[i].children.length; j++) {
    //   if (nodePath[i].children[j].id === nodePath[nodePath.length - 1].id) {
    //     nodePath[i].children.splice(j, 1);
    //   }
    // }
    ...
    for (let node of nodePath) {
      if (node.id === 25 && node.parent.id === 186) {
        console.log('here we go!!!');
      }
    }
  ...
  * _constructPath(actionNode, nodesToHit, callback) {
    ...
    while(validPath) {
      ...
      let actionToPeform;
      // Check if our target action is possible, choose immediatly if so
      // if (applicableActions.has(actionNode)) {
      //   actionToPeform = actionNode;
      ...
      if (!actionTree._visitedActions.has(actionToPeform.actionIdentifier)) {
        actionTree._visitedActions.add(actionToPeform.actionIdentifier);
      }
      actions.delete(actionToPeform.id);
      if (actionToPeform.actionIdentifier === actionNode) {
        break;
      }
    }
    ...
    if (node.children.length > 0) {
      ...
      for (let child of node.children) {
        if (child.parent.id !== node.id) {
          console.log('WJHAT TJHE FACK');
        }
        actionTree._treeTraversal(child, depth);
      }
    } else {
      actionTree._printTree(node, depth);
  ...
  _printTree(node, depth) {
    ...
    let path = [];
    while (node) {
      let spaceLengthNeeded = 100 - node.actionIdentifier.length;
      let spaceString = '';
      if (spaceLengthNeeded > 0) {
        for (let i = 0; i < spaceLengthNeeded; i++) {
          spaceString += ' ';
        }
      }
      count++;      
      if (!actionTree._uniqueWhilePrinting.has(node.id)) {
        actionTree._uniqueWhilePrinting.add(node.id);
        path.push(`\x1b[32m${node.actionIdentifier + spaceString + node.id}\x1b[0m`);
        // console.log(`\x1b[32m${count + ' ' + node.actionIdentifier + spaceString + node.id}\x1b[0m`);
      } else {
        // console.log(count + ' ' + node.actionIdentifier + spaceString + node.id);
        path.push(`${node.actionIdentifier + spaceString + node.id}`);
      }
    ...
    let pathNum = 0;
    for (let i = path.length - 1; i >= 0; i--) {
      console.log(pathNum + ' ' + path[i]);
      pathNum++;
    }

    if (count !== depth) {
      console.log('^^^^^^^^^^^^^^^^^^^^^^^^^');
      console.log('THIS ONE DID NOT ALIGN!!!');
      console.log(`depthPassed: ${depth}, pathLength: ${count}`);
    }
