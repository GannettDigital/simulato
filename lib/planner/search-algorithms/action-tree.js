...
module.exports = actionTree = {
  ...
  * _constructPath(actionNode, nodesToHit, callback) {
    ...
    // TODO: Before constructing a complicated path, check a short circuit easy path. For each action that solves our preconditions
    // Can we just throw the actionNode in the path right after one of those actions

    // Plug all notes to hit into a set to remove duplicates
    // TODO: Figure out why there are dupes (probably because two ids same identifier)
    ...
    // Check the sort circuit logic to see if we can easily add actionNode
