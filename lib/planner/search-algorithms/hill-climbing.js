...
module.exports = hillClimbing = {
    * createPlans(existingPlans, discoveredActions) {
        ...
        while (satisfiedActions.size < actionCounts.size) {
            let savedPlans = [];
            let startNodes = yield hillClimbing.emitAsync('startNodes.get', next);
            ...
            while (plan.path.length < maxPlanLength && satisfiedActions.size < actionCounts.size) {
                ...
                let {applicableActions} = yield hillClimbing.emitAsync('possibleActions.get', plan, next);
...
Emitter.mixIn(hillClimbing, plannerEventDispatch);
