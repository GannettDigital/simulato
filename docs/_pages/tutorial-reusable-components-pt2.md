...
As seen above, whenever we create a component of `type` `NewsArticle` we will need to pass in `baseId` inside the options. To help remember this its good practice to add some internal documentation at the top of each component that uses options. This will help other component creaters to reuse components more efficiently, and understand what is needed for the component you created.
