...
module.exports = hillClimbing = {
    * createPlans(existingPlans, discoveredActions) {
        let next = yield;
        let originalExistingPlans = existingPlans;
        let plans = [];
            ...
            while (plan.path.length < maxPlanLength && satisfiedActions.size < actionCounts.size) {
                ...
                if (plan.actions.size === 0) {
                    if (plan.path.length > 0) {
                        ...
                        throw new Error('No possible actions');
                    }
                }
                ...
                if (loopStartIndex !== -1) {
                    let backtrackState = yield hillClimbing.emitAsync('hillClimbing.backtrack', plan, originalExistingPlans, loopStartIndex, discoveredActions, next);
    ...
    * backtrack(plan, existingPlans, index, discoveredActions, callback) {
        let next = yield;

        let backtrackPlanFound = false;
        let backtrackState = null;

        while (index > 0 && !backtrackPlanFound) {
            backtrackState = yield hillClimbing.emitAsync('hillClimbing.createStateFromPath', plan.path.slice(0, index + 1), existingPlans, discoveredActions, next);
            let actions = new Set(backtrackState.plan.actions);

            actions.delete(backtrackState.plan.lastAction);
        ...
        callback(null, backtrackState);
    },
    * createStateFromPath(path, existingPlans, discoveredActions, callback) {
        let next = yield;
        ...
        return callback(null, {plan, savedPlans, satisfiedActions, actionCounts});
    }
};
...
hillClimbing.runOn('hillClimbing.createStateFromPath', hillClimbing.createStateFromPath);
